...
from reflective_learning.inference import (explore, explore_batched, sequence,
                                           sequence_batched)
from reflective_learning.metadata import callback, load, save
...
def run_explore(target, data, image, total, batch, device):
    ...
    info, *model = load(os.path.join(data, f"model.pt"), *state_space)
...
def run_play(goal, start, facing, model, total, batch, device):
    ...
    info, *model = load(model, *state_space)
